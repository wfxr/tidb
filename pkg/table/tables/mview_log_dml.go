...
type MLogDMLType byte

const (
	// MLogDMLTypeInsert writes DML type `I`.
	MLogDMLTypeInsert MLogDMLType = iota
...
func (t MLogDMLType) baseDMLType() string {
	switch t {
	case MLogDMLTypeInsert:
		return "I"
	case MLogDMLTypeUpdate:
		return "U"
	case MLogDMLTypeDelete:
		return "D"
	case MLogDMLTypeReplace:
		return "R"
	case MLogDMLTypeLoadData:
		return "L"
	default:
		// Should not happen; keep it explicit to avoid silently writing incorrect logs.
		return "?"
	}
}
...
func (t *mlogTable) shouldLogUpdate(touched []bool) bool {
	// For handle-changed updates, upper layer uses RemoveRecord + AddRecord, and we cannot get
	// `touched`. Here we only handle the handle-unchanged update path, where `touched` is
	// available.
	//
...
func (t *mlogTable) addRecordDMLType(opt *table.AddRecordOpt) string {
	if t.tp == MLogDMLTypeInsert && opt.IsUpdate() {
		return "U"
	}
	return t.tp.baseDMLType()
}

func (t *mlogTable) updateRecordDMLType() string {
...
func (t *mlogTable) removeRecordDMLType() string {
...
func (t *mlogTable) writeMLogRow(
	...
	mlogRow = append(mlogRow, types.NewStringDatum(dmlType), types.NewIntDatum(oldNew))
